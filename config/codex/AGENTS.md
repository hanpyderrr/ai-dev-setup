...
- 当完成当前任务所需的工具或依赖缺失时，如果预计下载量和最终新增存储占用均不超过 1 GB，默认允许直接下载安装并继续，无需逐项询问；任一项预计超过 1 GB 时，先说明用途和体积估算并征求用户确认。
- 当任务明确需要 OpenCLI 而本机尚未安装时，默认允许直接全局安装并继续；安装后先运行 `opencli list -f json` 核验可用命令，再按对应 skill 执行。
